    ...
    <>
      ...
      {fields.map((field, index) => (
        <Flex key={field.id} gap="4" items="start" mt="2">
          <FormFieldContext.Provider value={{ name: `variables.${index}.key` }}>
            <FormItem flex="1">
              <FormLabel>Key</FormLabel>
              <FormControl>
                <Input {...register(`variables.${index}.key`)} />
              </FormControl>
              <FormMessage />
            </FormItem>
          ...
          >
            <FormItem flex="1">
              ...
              <EncryptInformation index={index} control={control} />
              <FormMessage />
            </FormItem>
          </FormFieldContext.Provider>
...
const EncryptInformation = ({
  ...
  return (
    <FormDescription>
      {isSecret ? (
        <>
          This value <Strong>will no longer be viewable</Strong> once saved.
        </>
      ) : null}
    </FormDescription>
  );
