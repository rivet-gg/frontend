...
export default class DevGameLogs extends LitElement {
  ...
  logStream?: RepeatingRequest<
    Rivet.cloud.games.matchmaker.GetLobbyLogsResponse
  >;
  ...
  updated(changedProperties: PropertyValues) {
    ...
    if (
      changedProperties.has("logStreamType") ||
      changedProperties.has("lobbyData")
    ) {
      this.resetLogData();
  ...
  async fetchLobby(lobbyId: string) {
    ...
    try {
      ...
      this.logStreamType = this.lobbyData.lobby.status.stopped?.failed ?? false
  ...
  async streamLogs(lobbyId: string, logStreamType: cloud.LogStream) {
    ...
    this.logStream = new RepeatingRequest(
      "DevGameLogs.logStream",
      async (abortSignal, watchIndex) => {
        // TODO: Missing abort signal
        return await global.api.cloud.games.matchmaker.getLobbyLogs(
          this.game.gameId,
          lobbyId,
          {
            stream: logStreamType,
    ...
    if (this.logContent) {
      this.isFollowingLogs =
        this.logContent.scrollTop >=
          this.logContent.scrollHeight - this.logContent.clientHeight;
    }
  }
  ...
  render() {
    ...
    let selectedLobby = this.lobbyId
      ? this.lobbyData?.lobby ??
        this.lobbies.find((x) => x.lobbyId == this.lobbyId)
      : null;

    // Show placeholder if no lobbies and nothing is loading
    if (
      !this.isLoadingNamespace && !this.isLoadingLobby && !selectedLobby &&
    ...
    return html`
			...
			<div id="base">${this.renderSidebar()}${
  ...
  renderLog(lobby: cloud.LogsLobbySummary) {
    ...
    let regionData =
      this.game.availableRegions.find((r) => r.regionId == lobby.regionId) ??
        UNKNOWN_REGION;
            ...
            when(
              ...
              () =>
                html`
									...
									<div
										class=${
            classMap({ action: true, disabled: this.isExportingLogs })
          }
					...
					<code
						id="log-content"
						class=${
      classMap({ active: !this.isLoadingLogs && !this.isLogEmpty })
    }
      ...
      when(
        ...
        () =>
          html`<div class="muted-text">
								No ${
            this.logStreamType == cloud.LogStream.STD_OUT ? "stdout" : "stderr"
          }
  ...
  renderMetrics() {
    ...
    let maxMemory =
      Math.max(metrics.allocatedMemory, Math.max(...metrics.memory)) || 1;
    let timestamps = [...Array(metrics.cpu.length)].map((_, i) => {
      return new Date(
        Date.now() - (metrics.cpu.length - i) * timing.seconds(15),
      );
    });
    let maxMemoryPercent =
      maxMemory /
        (metrics.allocatedMemory == 0 ? maxMemory : metrics.allocatedMemory) ||
      1;
    ...
    let latestCPULabel = numbro(metrics.cpu[metrics.cpu.length - 1] / 100)
      .format("0.0%");
    let latestMemoryLabel = numbro(metrics.memory[metrics.memory.length - 1])
      .format("0.0 ib");
...
export function formatLobbyStatus(
  status: cloud.LogsLobbyStatus,
  startTs: Date,
) {
  return status.running !== undefined
    ? startTs ? "Running" : "Not Started"
    : status.stopped
    ? status.stopped.failed ? "Failed" : "Closed"
    : "Unknown status";
}
