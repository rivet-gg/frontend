...
type AuthServiceClientConfigType = PartialBy<Partial<__SmithyConfiguration<__HttpHandlerOptions>>
  & ClientDefaults
  & CustomEndpointsInputConfig
  & RetryInputConfig
  & HostHeaderInputConfig
...
type AuthServiceClientResolvedConfigType = __SmithyResolvedConfiguration<__HttpHandlerOptions>
...
export class AuthServiceClient extends __Client<
...
> {
  ...
  constructor(configuration: AuthServiceClientConfig) {
    ...
    endpoint: string | __Endpoint | __Provider<__Endpoint>, token: string
      ...
      if (endpoint === null) {
        try {
          endpoint = process.env.RIVET_AUTH_API_URL ?? null;
        }
        catch {
        }
        if (endpoint === null) {
      ...
      if (token === null) {
        try {
          token = process.env.RIVET_AUTH_TOKEN ?? process.env.RIVET_TOKEN ?? null;
        }
        catch {
        }
      }
