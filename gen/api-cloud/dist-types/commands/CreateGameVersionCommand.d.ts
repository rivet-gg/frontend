import { CloudServiceClientResolvedConfig, ServiceInputTypes, ServiceOutputTypes } from "../CloudServiceClient";
import { CreateGameVersionInput, CreateGameVersionOutput } from "../models/models_0";
import { Command as $Command } from "@aws-sdk/smithy-client";
import { Handler, MiddlewareStack, HttpHandlerOptions as __HttpHandlerOptions, MetadataBearer as __MetadataBearer } from "@aws-sdk/types";
export interface CreateGameVersionCommandInput extends CreateGameVersionInput {
}
export interface CreateGameVersionCommandOutput extends CreateGameVersionOutput, __MetadataBearer {
}
/**
 * Creates a new game version.
 * @example
 * Use a bare-bones client and the command you need to make an API call.
 * ```javascript
 * import { CloudServiceClient, CreateGameVersionCommand } from "@rivet-gg/cloud"; // ES Modules import
 * // const { CloudServiceClient, CreateGameVersionCommand } = require("@rivet-gg/cloud"); // CommonJS import
 * const client = new CloudServiceClient(config);
 * const command = new CreateGameVersionCommand(input);
 * const response = await client.send(command);
 * ```
 *
 * @see {@link CreateGameVersionCommandInput} for command's `input` shape.
 * @see {@link CreateGameVersionCommandOutput} for command's `response` shape.
 * @see {@link CloudServiceClientResolvedConfig | config} for CloudServiceClient's `config` shape.
 *
 */
export declare class CreateGameVersionCommand extends $Command<CreateGameVersionCommandInput, CreateGameVersionCommandOutput, CloudServiceClientResolvedConfig> {
    readonly input: CreateGameVersionCommandInput;
    constructor(input: CreateGameVersionCommandInput);
    /**
     * @internal
     */
    resolveMiddleware(clientStack: MiddlewareStack<ServiceInputTypes, ServiceOutputTypes>, configuration: CloudServiceClientResolvedConfig, options?: __HttpHandlerOptions): Handler<CreateGameVersionCommandInput, CreateGameVersionCommandOutput>;
    private serialize;
    private deserialize;
}
