...
type ChatServiceClientConfigType = PartialBy<Partial<__SmithyConfiguration<__HttpHandlerOptions>>
  & ClientDefaults
  & CustomEndpointsInputConfig
  & RetryInputConfig
  & HostHeaderInputConfig
...
type ChatServiceClientResolvedConfigType = __SmithyResolvedConfiguration<__HttpHandlerOptions>
...
export class ChatServiceClient extends __Client<
...
> {
  ...
  constructor(configuration: ChatServiceClientConfig) {
    ...
    endpoint: string | __Endpoint | __Provider<__Endpoint>, token: string
      ...
      if (endpoint === null) {
        try {
          endpoint = process.env.RIVET_CHAT_API_URL ?? null;
        }
        catch {
        }
        if (endpoint === null) {
      ...
      if (token === null) {
        try {
          token = process.env.RIVET_CHAT_TOKEN ?? process.env.RIVET_TOKEN ?? null;
        }
        catch {
        }
      }
