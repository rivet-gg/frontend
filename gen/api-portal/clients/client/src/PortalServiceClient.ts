...
type PortalServiceClientConfigType = PartialBy<Partial<__SmithyConfiguration<__HttpHandlerOptions>>
  & ClientDefaults
  & CustomEndpointsInputConfig
  & RetryInputConfig
  & HostHeaderInputConfig
...
type PortalServiceClientResolvedConfigType = __SmithyResolvedConfiguration<__HttpHandlerOptions>
...
export class PortalServiceClient extends __Client<
...
> {
  ...
  constructor(configuration: PortalServiceClientConfig) {
    ...
    endpoint: string | __Endpoint | __Provider<__Endpoint>, token: string
      ...
      if (endpoint === null) {
        try {
          endpoint = process.env.RIVET_PORTAL_API_URL ?? null;
        }
        catch {
        }
        if (endpoint === null) {
          endpoint = "https://portal.api.rivet.gg/v1";
        }
      }
      let token = input.token ?? null;
      if (token === null) {
        try {
          token = process.env.RIVET_PORTAL_TOKEN ?? process.env.RIVET_TOKEN ?? null;
        }
        catch {
        }
      }
